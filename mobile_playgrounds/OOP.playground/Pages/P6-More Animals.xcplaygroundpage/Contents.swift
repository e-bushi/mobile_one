...
class Bee: Animal {
    ...
    override func eat(food: String) {
        ...
        if food == favoriteFood {
            print("YUM!!! \(name) wants more pollen")
            sleep()
        } else {
            print("YUCK!!! \(name) will not eat \(food)")
        }
...
var tigger = Tiger(name: "Tigger")
tigger.eat(food: "meat")

var pooh = Bear(name: "Pooh")
pooh.eat(food: "fish")
pooh.eat(food: "meat")

var rarity = Unicorn(name: "Rarity")
rarity.eat(food: "marshmallows")
rarity.sleep()

var gemma = Giraffe(name: "Gemma")
gemma.eat(food: "meat")
gemma.eat(food: "leaves")

var stinger = Bee(name: "Stinger")
