...
class Car: Vehicle {
    ...
    init(maxSpeed: Int, numberOfWheels: Int, doors: Int, model: String) {
...
*/
var two_d = [[2, 5, 9], [0, 4, 2], [6, 8, 3]]

protocol Traverse {
    func multiIteration(_ twoDimensional: [[Any]]) -> [Any]
}

struct twoDimensionalArray: Traverse {
    
    func multiIteration(_ twoDimensional: [[Any]]) -> [Any] {
        var returnedArray = [Any]()
        
        var twoDimensionalIterator = twoDimensional.makeIterator()
        
        while let elements = twoDimensionalIterator.next() {
            for i in elements {
...
let two_dime = twoDimensionalArray()
two_dime.multiIteration(two_d)
