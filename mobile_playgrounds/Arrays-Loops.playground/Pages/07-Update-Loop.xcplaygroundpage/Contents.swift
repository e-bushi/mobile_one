...
*/
    ...
    override func update() {
        ...
        let numberOfColumns: Int = grid.count
        let numberOfRows: Int = grid[0].count
        
        
        
        for a in 0...(numberOfColumns - 1) {
            
            for b in 0...(numberOfRows - 1) {
                
                let numberOfAliveNeighbors = countNeighbors(grid: grid, column: a, row: b)
                
                if grid[a][b] == nil && numberOfAliveNeighbors == 2 {
                    
                    grid[a][b] = liveChar
                    
                } else if grid[a][b] != nil {
                    
                    let c = numberOfAliveNeighbors
