...
*/
    ...
    func getAlive(grid: [[Character?]], column x: Int, row y: Int) -> Int {
        ...
        let numOfColumns: Int = grid.count
        let numOfRows: Int = grid[0].count
        
        if (x >= numOfColumns || x < 0) {
            return 0
        } else if (y >= numOfRows || y < 0 ) {
            return 0
        }
...
*/
    ...
    override func countNeighbors(grid: [[Character?]], column x: Int, row y: Int) -> Int {
        ...
        let up = getAlive(grid: grid, column: x, row: (y - 1))
        
        let down = getAlive(grid: grid, column: x, row: y + 1)
        let right = getAlive(grid: grid, column: x + 1, row: y)
        let left = getAlive(grid: grid, column: x - 1, row: y)
        
        let topRight = getAlive(grid: grid, column: x + 1, row: y + 1)
        let topLeft = getAlive(grid: grid, column: x - 1, row: y + 1)
        let bottomLeft = getAlive(grid: grid, column: x - 1, row: y - 1)
        let bottomRight = getAlive(grid: grid, column: x + 1, row: y - 1)
        
        let arr: [Int] = [up, down, right, left, topRight, topLeft, bottomLeft, bottomRight]
